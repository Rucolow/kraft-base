...
#### 埋まり方（2026-07-25 送信開始・koguchi 申告）

- 直後は15分ごとの**差分**送信のため、直近に更新のあった注文だけが埋まる。
- JST深夜の洗い替えで**直近90日以内に更新のあった注文**がまとめて埋まる。
- それより古い注文は再送対象外で **null のまま**（過去分の全件送信はしない）。
- → 「送信開始したのに入っていない」時間帯が正常に存在する。確認は翌午前が確実。
...
- カナリア行: `customer_name = 'Kate Burrows '`（**末尾スペース**）,
  `source_updated_at = 2026-07-19T08:39:53.741+00:00`
- **到達すれば**: 末尾スペースが消え、`source_updated_at` が進む。
- 送信開始直後の確認時点では**未到達**（15分差分がまだ回っていない想定）。
  翌午前に未到達のままなら、差分送信が実際に成功しているかを koguchi に確認する。

配管の生死を「予約名が入ったか」で測ると、そもそも予約名が null の注文と区別できない。
**trim 済みか＋updatedAt が進んだか**で見るのが確実（値の有無に依存しない判定）。
