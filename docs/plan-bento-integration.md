...
1. **【最重要・運用】電話・直前注文も必ず koguchi 手打ちに入れる運用でよいか**
   （これが Yes でないと「正の一本化」が成立しない）
2. スタッフツールに出すのは KRAFT BASE 行きのみ（他宿INN分は出さない）でよいか
3. 弁当手入力欄は「注文以外のメモ専用」へ縮小し、2週間後にカウンター撤去、でよいか
4. 通しテストは koguchi の手打ち注文1件（決済なし）でよいか
5. 【モーリーに確認】「ベジタリアン弁当」（koguchi）と「ヴィーガン弁当」（スタッフツール）は
   同一商品か。正しい呼称はどちらか
