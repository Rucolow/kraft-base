...
ステータス: **全フェーズ完了・本番稼働中（2026-07-23）／契約 v3 完了（2026-07-25）**。
v3: reservation_name 追加・KB側実装マージ（PR #39）・カナリアで到達確認・未解決項目ゼロ。
...
- 自動照合: 完全一致のみ自動という案は koguchi から提案あり。未正規化データで誤爆＝
  ...
  必要になれば koguchi 側で正規化済みの比較用カラムを別に持って送る対応も可能（申し出あり）。

#### 埋まり方（2026-07-25 送信開始・koguchi 訂正反映）

**⚠️ 当初「洗い替えでまとまって埋まる」と記録したが誤り（koguchi が訂正）。**
洗い替えは行を再送するだけで、koguchi 側のDBでも既存注文の予約名は `null` のため
**何も埋まらない**。予約名は注文時に客が申告する項目で、機能実装前の注文には存在しない。

埋まるのは次の3経路のみ:

1. `/g` からの**新規注文**（客が「決済名と同じ」を選ぶ or 別名義を入力）
2. **運営者の手入力注文**（予約名欄が空なら注文者名を写すので KB宛は必ず埋まる）
3. 運営者が管理画面から**既存注文に後から入力**したとき

**既存注文は永久に `null` のまま。** koguchi は過去分を `customer_name` から自動補完
することを**意図的にしない**方針（技術的には可能）。理由は、それは客の申告値ではなく
当方の推測であり、誤爆の実害が「別の客に弁当を渡す」である以上、推測値を照合キーに
混ぜるべきではないため。**「値が入っている＝客または運営者が申告した」という意味を保つ。**
→ この不変条件があるので、将来 reservation_name を照合に使う際は
「入っている値は推測ではない」と信頼してよい（自動照合を検討する際の前提）。

#### ★入力率を測るときの落とし穴（自動照合の判断前に必読）

- 客の `/g` 注文: 「決済名と同じ」を選べば決済名と同一文字列、別名義なら入力値。
- **運営者の手入力注文（電話・直前）: 予約名欄が空なら注文者名が自動で入る。**
  つまり手入力分は**必ず埋まる**が、中身は「本人申告の予約名」ではなく
  スタッフが聞き取った名前。**入力率を素で集計すると手入力分で水増しされる** —
  自動照合のヒット率を評価するときは客注文分と手入力分を分けて見ること。
- カゴ落ち（決済前離脱）は null。
- 運営者が管理画面で予約名を直すと**即座に更新が push される**（後から変わる前提で読む）。

#### 自己修復の前提が保証されていることの確認（2026-07-25・koguchi 回答済み）

ウォーターマーク cron は「`updatedAt` が進んだ行」を拾い直すため、**運営者の後入力で
`updatedAt` が進まないと、イベント push が落ちた場合に永久に届かない**という silent
failure 経路を懸念して照会。回答は **Yes（自動で進む・対応不要）**:

- `updatedAt` は Prisma の `@updatedAt` 管理下で、アプリ経由の書き込みは自動更新。
  経路③（運営者の後入力）は `updateMany` なので自動で進む。
- **同じ値で上書きしても進む**（ORM は no-op をスキップしない）＝拾い直しは常に効く。
- 注文行の更新は全て ORM 経由に限る内部ルールがあり、回帰テスト
  `src/server/__tests__/kb-updated-at.test.ts` で固定（将来壊れたら向こうのCIで落ちる）。
- 例外は先日の一括 `btrim` のみ（SQL Editor の生SQLで ORM を迂回するため
  `updatedAt = now()` を明示した）。

→ **イベント push は best-effort のままでよい**という設計判断の根拠がこれで確定した。
push が落ちても cron が必ず拾い直す。この前提が崩れる変更が向こうに入った場合、
向こうのCIが検知する。

#### 送信開始の検証カナリア（2026-07-25）

koguchi が `customer_name`/`reservation_name` の trim を保存入口の共通正規化関数に統合し、
過去データにも一括 `btrim`（半角/全角/NBSP/タブ/改行）を適用、その際 `updatedAt` を
**明示的に進めた**と申告。よって到達判定は目視ではなく次の1行で確定できる:

- カナリア行 `cmrrjn26i0001in041w22kp5r`: 送信前は `customer_name = 'Kate Burrows '`
  （**末尾スペース**）, `source_updated_at = 2026-07-19T08:39:53.741+00:00`

**判定結果: 到達確認済み（2026-07-25 13:15 JST の差分送信）。**

| 項目 | 到達後の実測値 |
|---|---|
| `customer_name` | `'Kate Burrows'`（末尾スペース除去） |
| `source_updated_at` | `2026-07-25T04:03:22.693Z`（koguchi 申告と一致） |
| `synced_at`（KB側トリガー打刻） | `2026-07-25T04:15:29.987Z` |

`synced_at` が koguchi の watermark `04:15:29` と秒まで一致 = **両側から独立に裏取り**。
KB側のガードトリガーが正常発火して書き込みを受理したことも同時に確認できた。
最初の確認が未到達だったのは、単に 13:15 の送信直前に見ていたため。

**教訓: 配管の生死は「予約名が入ったか」で測ってはいけない。** 予約名は元々 null の
注文と区別がつかない（上記のとおり既存注文は永久に null）。**trim 済みか＋
source_updated_at が進んだか**という、値の有無に依存しない指標で見ること。

#### koguchi 側の在庫上限超過（2026-07-25 共有）

支払い済み注文の遅延復旧で、その日の確定数が**1日上限を超えうる**経路があると申告
（拒否せず在庫を戻す仕様のため超過が通知されていなかった。Slack警告を追加済み）。
KB側に不具合はないが、**「その日の確定数が上限超過」は正規に起こりうる**。
🍱パネルの食数が想定より多いときに、ツールの不具合と誤認しないこと。
上限値はミラーしていないため KB 側では超過を検知できない。

#### 実データ観測（2026-07-25）

決済名に `'Kate Burrows '`（末尾スペース）の実例あり。厳密一致で比較していたら
同一人物に「予約名」チップが誤表示されるところだった。表示判定を NFKC＋空白除去＋
小文字化の緩い比較にした判断は実データで裏取り済み。
