...
|---|---|---|---|
| 真偽値 | integer 0/1 | boolean | `lib/powersync/serialize.ts`（ブランチ、ステージング検証待ち） |
...
| staff 自己claim | 未claim行なら誰でも (0011)。**owner行も奪える穴 → 0015で封鎖、未適用** | LinkAccountのUIガードはブランチ済み |
...
2. demo検証: `apps/staff/e2e/` の該当スイート＋sweep（コンソールエラー0）。
3. **フロントのみ**の変更 → PR→CI緑→master マージ（Vercelが本番 `kraft-base-staff` を自動デプロイ）。
4. **同期/RLS/認証層** → 必ずステージング（`supabase/staging/VERIFY-sync-rls.md`）で実証してから。
   本番DBへのSQL適用はオーナーの手で（AIは本番DBに触れない）。マイグレーションは連番で
   `apps/staff/supabase/migrations/` に置き、PR本文に適用SQLを明記。
...
| migrations 追加全般 | `supabase/staging/schema.sql`（結合再生成）＋ステージングにも適用 |
