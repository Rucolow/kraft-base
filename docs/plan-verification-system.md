...
2. **既存テストの決定的バグ（CIが炙り出した・3件）**: タイミング競合ではなく、
   セレクタ/文字列一致のバグ。これまでスイート未走のため潜在化していた（＝CI化の意義そのもの）。
   - `roundtrip.cjs`「edit: country prefilled」→ `getByPlaceholder('自由入力')` が
     `言語（自由入力）` にも部分一致し strict違反→''。**`{ exact: true }`** で解決。
   - `interact.cjs`「products: edit mode shows 商品名」→ `商品名` は**placeholder**で
     innerTextに出ない（README地雷#5）。**`商品を追加` ラベル**を待って判定に変更。
   - `interact.cjs`「STAFF products: NO 編集 button」→ `has(sp,'編集')` が staff向け
     ヘルプ文 `価格の編集はオーナーのみ。` に誤マッチ。**編集ボタン数=0** で判定に変更。
3. **教訓**: これらは環境非依存で常に落ちていたはずのテスト。CIに載せて初めて可視化された
   （＝「誰が作業してもブラウザ検証が必ず走る」効果の実証）。README地雷#4/#5の再発。
