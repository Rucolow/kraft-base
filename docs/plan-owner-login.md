...
### P0-6（追補 2026-07-09）: ログイン成功後に /login から自動遷移する

実戦で確定した潜在バグ: `verifyCode` 成功後、誰も画面遷移させない。
Login.tsx は success 時「onAuthStateChange で app が routes on する」とコメントして
いるが、`/login` ルートは RequireApp の外にあり、セッションが立っても Login が
表示され続ける。ユーザーには「ログインを押しても何も起こらない」に見え、
もう一度押すと**使用済みコードの再検証**で「正しくない」エラーになる
（成功が失敗に見える最悪のUX）。マジックリンク経由や再読み込みでは偶然
`/` に着地するため、これまで発覚しなかった。

- Login に `useAuth().session` を購読する `useEffect` を追加し、
  `session` が非 null になったら `navigate('/', { replace: true })`。
  （verify 成功時の直接 navigate より、リンク着地・既ログイン再訪も
  カバーできるこの形が正しい）
- **受け入れ条件**: vitest ではなく e2e/手動: demo モードは auth 無しのため、
  ユニットは mapAuthError 同様の切り出しが不要なら省略可。実機確認では
  コード入力→ログイン押下→自動で / へ遷移すること。
